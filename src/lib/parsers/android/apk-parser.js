 ...
 * @typedef {import('./types.js').AndroidNativeLib} AndroidNativeLib
 * @typedef {import('./types.js').AndroidResource} AndroidResource
 * @typedef {import('../common/zip-parser.js').ZIPEntry} ZIPEntry
 */

/**
 * Parse APK file and extract all metadata
 * @param {File} file - APK file to parse
 ...
 * @returns {Promise<APKParseResult>} Parsed APK result
 */
export async function parseAPKFromEntries(entries, originalFileSize) {
 ...
 * @param {ZIPEntry[]} entries - ZIP entries
 * @returns {import('./dex-parser.js').DEXMetadata[]} DEX metadata array
 */
function parseDEXFiles(entries) {
  const dexFiles = [];
 ...
 * @param {ZIPEntry[]} entries - ZIP entries
 * @returns {AndroidNativeLib[]} Native libraries
 */
function parseNativeLibs(entries) {
  const libs = [];
 ...
 * @param {ZIPEntry[]} entries - ZIP entries
 * @returns {AndroidResource[]} Resources
 */
function parseResources(entries) {
  const resources = [];
 ...
 * @param {ZIPEntry[]} entries - ZIP entries
 * @returns {Array<{path: string, size: number}>} Assets
 */
function extractAssets(entries) {
  return entries
 ...
 * @param {AndroidNativeLib[]} nativeLibs - Native libraries
 * @returns {string[]} Architectures
 */
function extractArchitectures(nativeLibs) {
